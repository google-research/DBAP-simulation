...
from adept_envs.simulation.dm_renderer import DMRenderer
from adept_envs.simulation.sim_scene import SimScene
# GOOGLE3-ONLY from adept_envs.utils.resources import AssetBundle
# GOOGLE3-ONLY from google3.robotics.reinforcement_learning.mujoco.license_key_cache import mujoco_license_cache # pylint: disable=line-too-long
...
class DMSimScene(SimScene):
    ...
    def _load_simulation(self, model_handle: Any) -> Any:
        ...
        """
        # GOOGLE3-ONLY # Use the user's Brain MuJoCo license.
        # GOOGLE3-ONLY mujoco_license_cache.register_user_key()
        if isinstance(model_handle, str):
            if model_handle.endswith('.xml'):
                # GOOGLE3-ONLY bundle = AssetBundle()
                # GOOGLE3-ONLY model_handle = bundle.add_mujoco(model_handle)
                sim = dm_mujoco.Physics.from_xml_path(model_handle)
                # GOOGLE3-ONLY bundle.cleanup()
            else:
