...
class FrankaMicrowaveCabinetSlider(BaseFrankaEnv):
    ...
    def __init__(self,
        ...
        if self._play_file:
            self.play_data = pickle.load(open(self._play_file, 'rb'))
    ...
    def reset(self):
        ...
        idx_goal = np.random.choice(range(len(self.play_data['observations'])))
        qp_start = 0
        qp_end = self.get_obs_dict()['qp'].shape[0] + self.get_obs_dict()['obj_qp'].shape[0]
        mocap_end = self.get_obs_dict()['qp'].shape[0] + \
                    self.get_obs_dict()['obj_qp'].shape[0] + \
                    self.get_obs_dict()['mocap_pos'].shape[0] +
        self.sim.data.qpos[:] = self.play_data['observations'][idx_goal, qp_start:qp_end]
        self.sim.data.mocap_pos[:] = self.play_data['observations'][idx_goal, qp_end:mocap_end]
        for _ in range(100):
            self.sim.step()
        self.goal = self.play_data['observations'][idx_goal + 100, :self.goal_space.shape[0]]
    ...
    def get_reward_dict(self,
        ...
        slider_error = self._get_obs()[:self.goal_space.shape[0]] - self.goal[:self.goal_space.shape[0]]
        reward_dict = collections.OrderedDict((
            ('ee_slider', np.array([np.float(np.linalg.norm(slider_error) < 0.4)])),
        ))
        return reward_dict