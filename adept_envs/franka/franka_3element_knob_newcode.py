...
ASSET_PATH = 'adept_envs/franka/assets/franka_knob_cabinet_slider_only3elements.xml'
import os
import sys
...
class Franka3ElementKnob(BaseFrankaEnv):
    ...
    def __init__(self,
        ...
        # self.goal_matrix = pickle.load(open('./goal_matrix_3elements.pkl', 'rb'))
        self.goal_matrix = np.zeros((8, 8))

        for i in range(8):
            curr_idx = np.binary_repr(i)
            curr_idx = '0' * (3 - len(curr_idx)) + curr_idx
            curr_idx = np.array([int(curr_idx[k]) for k in range(len(curr_idx))])
            for j in range(3):
                new_idx = copy.deepcopy(curr_idx)
                new_idx[j] = (new_idx[j] + 1) % 2
                new_idx_val = np.sum(np.array([2 ** i for i in range(3)])[::-1] * new_idx)
        ...
        self._counts = np.zeros(8, )
        self._goals = pickle.load(open('./3element_knob_goal_idxs.pkl', 'rb'))
        self._goals_val = pickle.load(open('./3element_knob_goal_vals.pkl', 'rb'))
        self.eps = 0.4

        self.density = np.zeros((8,))
    ...
    def learned_goal_select(self, goal_selected):
        if self.learned_model:
            ...
            o = self._get_obs(self.get_obs_dict())[2:5]
            if self._reset_counter % 8 < 4:
                next_task = np.random.randint(8)
            else:
                next_task = 0
            ...
            goal_selected = np.random.choice(range(8), p=output_x)
            print("LEARNED LIKELIHOOD PREDICTIONS " + str(output_x))

        # Updating counts
        curr_count = np.zeros((8,))
    ...
    def reset_arm_state(self, start_idx):
        ...
        new_qpos[9:12] = self._goals_val[start_idx][2:5]  # Reset to a particular state

        self.sim.data.qpos[:] = new_qpos.copy()
        self.sim.data.mocap_pos[:] = np.array([-0.05164342, 0.05300494,
                                               2.62070813])  # self._goals_val[start_idx][5:8] #np.array([-0.16922002,  0.07353752,  2.57067996])
        for _ in range(100):
            self.sim.step()
            self.robot.step({
                'gripper': 1 * np.ones(2)
    ...
    def reset_arm_only(self):
        ...
        self.sim.data.qpos[:] = new_qpos.copy()
        self.sim.data.mocap_pos[:] = np.array([-0.05164342, 0.05300494,
                                               2.62070813])  # self._goals_val[self._goal_idx][5:8] #np.array([-0.16922002,  0.07353752,  2.57067996])
        for _ in range(100):
            self.sim.step()
            self.robot.step({
                'gripper': 1 * np.ones(2)
    ...
    def reset(self):
        ...
        if self._reset_counter > 0 and self.graph_search:
            ...
            self.update_graph(self._current_idx,
                              current_end_state)  # Current idx is the state at the start of the epiisode,
    ...
    def get_obs_dict(self):
        ...
        curr_idx = self.check_goal_completion(self.sim.data.qpos[-self.N_DOF_OBJ:])
        completion = np.array([np.float(curr_idx == self._goal_idx)])
    ...
    def select_goal(self):
        ...
        density_diffs = [-np.inf for i in range(8)]
        ...
        for i in range(8):
            ...
            # TVD to the uniform
            density_diffs[i] = -np.sum(np.abs(new_density - np.ones(8) * (1 / 8.)))
    ...
    def get_reward_dict(
        ...
        if self.idx_completion:
            ...
            self.microwave_arm_pos = self.sim.data.site_xpos[self.sim.model.site_name2id('light_site')]

            self.slider_open_pos = self._goals_val[self._goal_idx][2]
            self.microwave_open_pos = self._goals_val[self._goal_idx][3]
            self.cabinet_open_pos = self._goals_val[self._goal_idx][4]

            self.slider_closed_pos = self._goals_val[self._goal_idx][2]
            self.microwave_closed_pos = self._goals_val[self._goal_idx][3]
            ...
            goal_idx = '0' * (3 - len(goal_idx)) + goal_idx
            curr_idx = '0' * (3 - len(curr_idx)) + curr_idx
