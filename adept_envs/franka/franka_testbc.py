...
class FrankaMicrowaveCabinetSlider(BaseFrankaEnv):
    ...
    def __init__(self,
        ...
        super().__init__(
            ...
            **kwargs)
        self.commanded_start = 0
        self.commanded_goal = 2
    ...
    def reset(self):
        ...
        if self._eval_mode or self.reset_counter == 0 or \
            ...
            curr_goal_idx = self.commanded_start #np.random.randint(4)
            ...
            next_index = self.commanded_goal #np.random.choice(np.where(self.adjacency_matrix[curr_goal_idx] > 0)[0])
            next_li = 0 #np.random.choice(np.arange(len(self.labeled_goals[next_index])))
            self.goal = self.labeled_goals[next_index][next_li][:13]
    ...
    def check_goal_completion(self, curr_pos):
        max_objs = np.array([0.2, 1, 0.6, -0.05])
        min_objs = np.array([0.1, 0.1, 0.2, -0.2])
        ...
        if not self._idx_completion:
            if self.goal_idx == 0:
                if self.current_idx == 2 or self.current_idx == 0:
                    ...
                    slider_error = obs_dict['obj_qp'][:, 0:1] - self.goal[2:3]
                    reward_dict = collections.OrderedDict((
                        ('ee_slider', np.array([-50*np.float(np.linalg.norm(slider_error))])),
                    ...
                    arm_error = obs_dict['mocap_pos'] - target_pos
                    slider_error = obs_dict['obj_qp'][:, 2:3] - self.goal[4:5]
                    reward_dict = collections.OrderedDict((
                        ('ee_slider', np.array([-50*np.float(np.linalg.norm(slider_error))])),
                        ('arm_dist', np.array([-np.float(np.linalg.norm(arm_error))])),
                    ))
