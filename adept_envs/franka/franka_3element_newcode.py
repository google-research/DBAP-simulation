...
import pickle
ASSET_PATH = 'adept_envs/franka/assets/franka_microwave_cabinet_slider_only3elements.xml'
import os
import sys
...
class Franka3Element(BaseFrankaEnv):
    ...
    def __init__(self,
        ...
        self._goals = pickle.load(open('./3element_goal_idxs.pkl', 'rb'))
        self._goals_val = pickle.load(open('./3element_goal_vals.pkl', 'rb'))
        self.eps = 0.4
    ...
    def reset_arm_state(self, start_idx):
        ...
        self.sim.data.mocap_pos[:] = np.array([-0.05164342,  0.05300494,  2.62070813]) #self._goals_val[start_idx][5:8] #np.array([-0.16922002,  0.07353752,  2.57067996])
        for _ in range(100):
            self.sim.step()
            self.robot.step({
                'gripper': 1 * np.ones(2)
    ...
    def reset_arm_only(self):
        ...
        self.sim.data.qpos[:] = new_qpos.copy()
        self.sim.data.mocap_pos[:] = np.array([-0.05164342,  0.05300494,  2.62070813]) #self._goals_val[self._goal_idx][5:8] #np.array([-0.16922002,  0.07353752,  2.57067996])
        for _ in range(100):
            self.sim.step()
            self.robot.step({
                'gripper': 1 * np.ones(2)
    ...
    def get_reward_dict(
        ...
        if self.idx_completion:
            ...
            self.microwave_arm_pos = self.sim.data.site_xpos[self.sim.model.site_name2id('microhandle_site')]
